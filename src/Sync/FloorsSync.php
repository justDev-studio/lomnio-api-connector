<?php
/**
 * Floors endpoint synchronization.
 *
...
final class FloorsSync {
	private const ENDPOINT_KEY    = 'floors';
	private const ACTION_HOOK     = 'lomnio_api_connector_sync_floors';
	...
	private const SCHEDULE_OPTION = 'lomnio_api_connector_floors_schedule_signature';
	private const LOCK_RESOURCE   = 'floors';
	private const API_BASE_URL    = 'https://app.lomnio.com/api';
	private const API_LIST_PATH   = '/v1/floors';
	...
	public function __construct( SecretStorage $secret_storage, FloorRepository $repository, ?SyncLock $sync_lock = null ) {
	...
	public function run() {
		...
		if ( empty( $headers ) ) {
			$error = new \WP_Error(
				'lomnio_floors_missing_api_token',
		...
		if ( ! $acquired ) {
			$this->store_pending_meta( __( 'Floors sync skipped: another floors write is already in progress.', 'lomnio-api-connector' ), 'Skipped' );
			return true;
		}
	...
	/**
	 * Queue immediate floors sync after token save.
	 */
	public function run_after_api_token_saved(): void {
		$this->ensure_scheduled();

		$settings = $this->floors_settings();

		if ( empty( $settings['active'] ) ) {
			$this->store_pending_meta( __( 'API token saved. Floors sync is inactive.', 'lomnio-api-connector' ), 'Inactive' );
			return;
		}
	...
	public function enqueue_immediate(): void {
		if ( $this->action_scheduler_async_available() ) {
			...
			if ( $action_id ) {
				$this->store_pending_meta( __( 'Floors sync queued after API token save.', 'lomnio-api-connector' ), 'Queued' );
				return;
			}
		}
	...
	private function sync_list( array $headers ) {
		...
		if ( $status_code < 200 || $status_code >= 300 ) {
			$message = sprintf(
				...
				__( 'Lomnio floors list request failed with HTTP %d.', 'lomnio-api-connector' ),
				$status_code
			);
			$this->store_meta( false, $message );
			return new \WP_Error( 'lomnio_floors_http_error', $message );
		}
		...
		if ( ! is_array( $decoded ) || ! isset( $decoded['data'] ) || ! is_array( $decoded['data'] ) ) {
			$error = new \WP_Error(
				'lomnio_floors_invalid_json',
				__( 'Lomnio floors list response does not match the expected shape.', 'lomnio-api-connector' )
			);
			$this->store_meta( false, $error->get_error_message() );
			return $error;
		}

		$stored = $this->repository->store_list_floors( $decoded['data'] );
		...
		$this->store_meta(
			true,
			sprintf(
				/* translators: %d: synced floors count. */
				__( 'Floors synchronized successfully. Records processed: %d.', 'lomnio-api-connector' ),
				count( $stored )
			)
		);
	...
	/**
	 * Get stored floors endpoint settings.
	 */
	private function floors_settings(): array {
		...
		$floors = isset( $settings[ self::ENDPOINT_KEY ] ) && is_array( $settings[ self::ENDPOINT_KEY ] )
			? $settings[ self::ENDPOINT_KEY ]
			: array();

		return array(
			'active'   => array_key_exists( 'active', $floors ) ? (bool) $floors['active'] : true,
			'schedule' => ! empty( $floors['schedule'] ) ? sanitize_key( (string) $floors['schedule'] ) : 'every_ten_minutes',
		);
	}
